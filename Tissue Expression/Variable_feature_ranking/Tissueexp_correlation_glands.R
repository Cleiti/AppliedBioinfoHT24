...
rows_to_remove <- c()
for (i in 1:nrow(LASC_data_glands)) { 
...
rows_to_remove <- c()
for (i in 1:nrow(clavata_data_glands)) { 
...
common_rows <- intersect(rownames(log_LASC_data_glands), rownames(log_clavata_data_glands))
log_LASC_data_glands_filtered <- log_LASC_data_glands[common_rows, , drop = FALSE]
log_clavata_data_glands_filtered <- log_clavata_data_glands[common_rows, , drop = FALSE]
# calculating feature variances
feature_variances_lasc <- apply(log_LASC_data_glands_filtered, 1, var, na.rm=TRUE)
feature_variances_clavata <- apply(log_clavata_data_glands_filtered, 1, var, na.rm=TRUE)
...
correlation_plot <- ggplot(merged_df, aes(x = rank_LASC, y = rank_clavata)) +
  ...
  labs(x = "rank_lasc", y = "rank_clavata", 
       title = "Correlation of Orthogroup Variance Ranking in comparison of MAG to Aggregate gland") +
  theme_minimal() +
