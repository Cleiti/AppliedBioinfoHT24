...
data <- Read10X(data.dir = "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_data") # folder with 3 files
obj <- CreateSeuratObject(counts = data, project = "scRNA_t.clavipes")
...
obj_filtered <- subset(obj_filtered, subset = nFeature_RNA < 800)
plot_QC(obj_filtered)

#---------------------------------------------------------------------
annotation <- read.table("C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_gene_to_protein_to_assemblysequenceID_to_OG.tsv", sep = "\t", header = TRUE)
annotation <- annotation[, c(1,4)]
# filtering features by genes that:
#                                   are present in the NepCla reference genome
#                                 & in the transcriptomic assembly
#                                 & in the filtered (!) orthofinder dataset

# filtering by genes only present in filtered orthofinder
features_filtered_out <- c()
for (geneID in rownames(obj_filtered)) {
  if (geneID %in% annotation$gene_id) {
    rowname_annotation <- which(annotation$gene_id == geneID)
    if (is.na(annotation[rowname_annotation, "OG"])) {
...
# -> 12830 features left (2/3 of total removed)
plot_QC(obj_filtered_by_OG)

# ANALYSIS CONTINUED: ---------------------------------------------DoubletFinder

# OBS: running once per whole dataset because it's all one sample (one spider)
...
saveRDS(obj, "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/clavipes_analyzed.rds")
...
write.csv(markers_res.0.1, file = "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.1.csv", row.names = TRUE)
write.csv(markers_res.0.2, file = "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.2.csv", row.names = TRUE)
write.csv(markers_res.0.3, file = "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.3.csv", row.names = TRUE)
write.csv(markers_res.0.4, file = "C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.4.csv", row.names = TRUE)


#--------------------- annotation via OG comparison to LASC celltypes
annotation <- read.table("C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_gene_to_protein_to_assemblysequenceID_to_OG.tsv", sep = "\t", header = TRUE)
annotation <- annotation[, c(1,4)]
OGs <- annotation # for clavipes

# prepare lsac cluster information
...
markers_res.0.1 <- read.csv("C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.1.csv", header = TRUE, sep = ",")
...
markers_res.0.1 <- as.data.frame(markers_res.0.1 %>% 
                         ...
                         ungroup())
# add OG for each gene using clavipes annotation file (translation table)
...
clavipes_res.0.1_OGs_per_cluster <- split(markers_res.0.1$OG, markers_res.0.1$cluster)
clavipes_res.0.1_OGs_per_cluster <- lapply(clavipes_res.0.1_OGs_per_cluster, function(x) x[!is.na(x)])
# calculate overlap between each cluster in clavipes res.0.1 and lasc
calculate_overlap <- function(genes1, genes2) {
  length(intersect(genes1, genes2))
}
overlap_matrix_res.0.1 <- outer(
  names(clavipes_res.0.1_OGs_per_cluster), names(lasc_OGs_per_cluster), 
  Vectorize(function(cl1, cl2) calculate_overlap(clavipes_res.0.1_OGs_per_cluster[[cl1]], lasc_OGs_per_cluster[[cl2]]))
)
# set rownames and colnames as cluster names
overlap_df_res.0.1 <- as.data.frame(overlap_matrix_res.0.1, row.names = names(clavipes_res.0.1_OGs_per_cluster))
...
pheatmap(overlap_df_res.0.1, 
         ...
         main = "Celltype overlaps: t.clavipes res.0.1 vs l.sclopetarius", xlab = "LASC celltypes", ylab = "Clavipes res.0.1 clusters",
         color = colorRampPalette(c("lightblue", "red"))(50))

# ----- clustering res.0.4 ------
# select columns from cluster biomarkers and ensure correct sorting
markers_res.0.4 <- read.csv("C:/Users/Cleiti/Desktop/Seurat_spiders/clavipes/clavipes_results/markers_res.0.4.csv", header = TRUE, sep = ",")
markers_res.0.4 <- markers_res.0.4[, c("p_val_adj", "cluster", "gene")]
markers_res.0.4 <- markers_res.0.4[order(markers_res.0.4$cluster, markers_res.0.4$p_val_adj), ] # making sure that it's sorted the right way
# subsetting top 50 genes per cluster
markers_res.0.4 <- as.data.frame(markers_res.0.4 %>% 
                                   group_by(cluster) %>%
                                   slice_min(order_by = p_val_adj, n = 50) %>%  
                                   ungroup())
# add OG for each gene using clavipes annotation file (translation table)
markers_res.0.4$OG <- NA
for (i in 1:length(markers_res.0.4[,1])) {
  gene <- markers_res.0.4[i,3]
  if ((!is.na(gene)) && gene %in% OGs[,1]) {
    row <- which(OGs[,1] == gene)
    OG <- OGs[row, 2]
    markers_res.0.4[i,4] <- OG
  }
}
# create a nested list of OGs in each cluster
clavipes_res.0.4_OGs_per_cluster <- split(markers_res.0.4$OG, markers_res.0.4$cluster)
clavipes_res.0.4_OGs_per_cluster <- lapply(clavipes_res.0.4_OGs_per_cluster, function(x) x[!is.na(x)])
# calculate overlap between each cluster in clavipes res.0.4 and lasc
overlap_matrix_res.0.4 <- outer(
  names(clavipes_res.0.4_OGs_per_cluster), names(lasc_OGs_per_cluster), 
  Vectorize(function(cl1, cl2) calculate_overlap(clavipes_res.0.4_OGs_per_cluster[[cl1]], lasc_OGs_per_cluster[[cl2]]))
)
# set rownames and colnames as cluster names
overlap_df_res.0.4 <- as.data.frame(overlap_matrix_res.0.4, row.names = names(clavipes_res.0.4_OGs_per_cluster))
colnames(overlap_df_res.0.4) <- names(lasc_OGs_per_cluster)
# make a heatmap
pheatmap(overlap_df_res.0.4, 
         cluster_rows = FALSE, cluster_cols = FALSE, display_numbers = TRUE, 
         main = "Celltype overlaps: t.clavipes res.0.4 vs l.sclopetarius",
         color = colorRampPalette(c("lightblue", "red"))(50))
